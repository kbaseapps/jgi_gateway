package jgi_gateway::jgi_gatewayClient;
...
sub new
{
    ...
    my $self = {
	client => jgi_gateway::jgi_gatewayClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$parameter is a jgi_gateway.SearchInput
$result is a jgi_gateway.SearchResult
$error is a jgi_gateway.Error
$stats is a jgi_gateway.CallStats
SearchInput is a reference to a hash where the following keys are defined:
	query has a value which is a jgi_gateway.SearchQuery
	filter has a value which is a jgi_gateway.SearchFilter
	sort has a value which is a reference to a list where each element is a jgi_gateway.SortSpec
	limit has a value which is an int
	page has a value which is an int
	include_private has a value which is a jgi_gateway.bool
...
SearchResult is a reference to a hash where the following keys are defined:
	search_result has a value which is a jgi_gateway.SearchQueryResult
SearchQueryResult is a reference to a hash where the following keys are defined:
	hits has a value which is a reference to a list where each element is a jgi_gateway.SearchResultItem
	total has a value which is an int
SearchResultItem is a reference to a hash where the following keys are defined:
	source has a value which is a jgi_gateway.SearchDocument
...
<pre>
$parameter is a jgi_gateway.StageInput
$result is a jgi_gateway.StagingResult
$error is a jgi_gateway.Error
$stats is a jgi_gateway.CallStats
StageInput is a reference to a hash where the following keys are defined:
	file has a value which is a jgi_gateway.StageRequest
StageRequest is a reference to a hash where the following keys are defined:
	id has a value which is a jgi_gateway.JamoID
...
<pre>
$parameter is a jgi_gateway.StagingStatusInput
$result is a jgi_gateway.StagingStatusResult
$error is a jgi_gateway.Error
$stats is a jgi_gateway.CallStats
...
# Authentication: required
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "jgi_gateway.stage_status",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
<pre>
$parameter is a jgi_gateway.StagingJobsInput
$result is a jgi_gateway.StagingJobsResult
$error is a jgi_gateway.Error
$stats is a jgi_gateway.CallStats
StagingJobsInput is a reference to a hash where the following keys are defined:
	filter has a value which is a jgi_gateway.StagingJobsFilter
	range has a value which is a jgi_gateway.StagingJobsRange
	sort has a value which is a reference to a list where each element is a jgi_gateway.SortSpec
StagingJobsFilter is a reference to a hash where the following keys are defined:
	created_from has a value which is a jgi_gateway.timestamp
	created_to has a value which is a jgi_gateway.timestamp
	updated_from has a value which is a jgi_gateway.timestamp
	updated_to has a value which is a jgi_gateway.timestamp
...
StagingJobsResult is a reference to a hash where the following keys are defined:
	staging_jobs has a value which is a reference to a list where each element is a jgi_gateway.StagingJob
...
StagingJob is a reference to a hash where the following keys are defined:
	...
	job_monitoring_id has a value which is a string
	created has a value which is a jgi_gateway.timestamp
	updated has a value which is a jgi_gateway.timestamp
Error is a reference to a hash where the following keys are defined:
	message has a value which is a string
	type has a value which is a string
	code has a value which is a string
...
# Authentication: required
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "jgi_gateway.staging_jobs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
<pre>
$parameter is a jgi_gateway.StagingJobsSummaryInput
$result is a jgi_gateway.StagingJobsSummaryResult
$error is a jgi_gateway.Error
$stats is a jgi_gateway.CallStats
...
StagingJobsSummaryResult is a reference to a hash where the following keys are defined:
	states has a value which is a reference to a hash where the key is a string and the value is a jgi_gateway.StagingJobsSummary
	ids_states has a value which is a reference to a hash where the key is a string and the value is a reference to a hash where the key is a jgi_gateway.JamoID and the value is a jgi_gateway.StagingJobsSummary
...
# Authentication: required
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "jgi_gateway.staging_jobs_summary",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
<pre>
$parameter is a jgi_gateway.RemoveStagingJobInput
$result is a jgi_gateway.RemoveStagingJobResult
$error is a jgi_gateway.Error
$stats is a jgi_gateway.CallStats
...
# Authentication: required
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "jgi_gateway.remove_staging_job",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for jgi_gateway::jgi_gatewayClient\n";
    }
    if ($sMajor == 0) {
        warn "jgi_gateway::jgi_gatewayClient version is $svr_version. API subject to change.\n";
    }
}
...
package jgi_gateway::jgi_gatewayClient::RpcClient;
